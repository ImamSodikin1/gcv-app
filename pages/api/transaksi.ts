...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const { data, error } = await supabase.from('transaksi').insert(req.body).select().single();
            if (error) throw error;
        ...
        if (req.method === 'PUT') {
            ...
            const { data, error } = await supabase.from('transaksi').update(updates).eq('id', id).select().single();
            if (error) throw error;
