...
interface GroupedTableProps<TData extends Record<string, any>> {
  ...
  groupByKey: string;
  groupByLabel?: string;
  groupLabelFn?: (value: string) => string;
}

export function GroupedTable<TData extends Record<string, any>>({
  columns,
  data,
  isDark,
  isLoading = false,
  ...
  const groupedData = React.useMemo(() => {
    ...
    data.forEach((item) => {
      const key = String(item[groupByKey] || 'Tidak Ada');
      if (!groups[key]) {
        groups[key] = [];
      }
  ...
  return (
    <div className={`rounded-lg overflow-hidden shadow-2xl ${isDark ? 'bg-[#181926]/50 border-2 border-purple-500/30' : 'bg-white border-2 border-blue-200'}`}>
        ...
        <table className="w-full min-w-[640px]">
          <thead>
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className={`bg-gradient-to-r ${isDark ? 'from-purple-600/30 via-blue-600/30 to-cyan-600/30 border-b-2 border-purple-500/30' : 'from-blue-500/10 via-purple-500/10 to-pink-500/10 border-b-2 border-blue-200'}`}>
          ...
          <tbody className={`divide-y ${borderColor}`}>
            {table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row, idx) => {
                const groupValue = String(row.original[groupByKey] || 'Tidak Ada');
                const isFirstInGroup = idx === 0 || String(table.getRowModel().rows[idx - 1].original[groupByKey]) !== groupValue;

                return (
                  <React.Fragment key={row.id}>
                    {isFirstInGroup && (
                      <tr className={`${isDark ? 'bg-purple-600/10 border-y border-purple-500/20' : 'bg-blue-50 border-y border-blue-200'}`}>
                        <td colSpan={columns.length} className={`px-4 py-3 font-bold text-sm ${isDark ? 'text-purple-200' : 'text-blue-900'}`}>
                          {groupByLabel}: <span className="font-mono">{groupLabelFn ? groupLabelFn(groupValue) : groupValue}</span>
                        </td>
                      </tr>
                    )}
                    <tr
      ...
      {/* Pagination */}
      <div className={`flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-4 border-t-2 ${borderColor} bg-opacity-50`}>
